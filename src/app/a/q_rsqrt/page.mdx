...
The remainder of this article will be an analysis of the Quake III Arena code, which can be found [here](https://github.com/id-Software/Quake-III-Arena/blob/dbe4ddb10315479fc00086f08e25d968b4b43c49/code/game/q_math.c#L552). For reference, here is a slightly modified (functionally identical) version of the function:
...
- If the exponent field is all ones and the mantissa is not all zeros, the value represented is not a number (`NaN`).
...
Since the floating-point values that fast inverse square root is designed to work with are always positive (the square root of a negative number is imaginary), the sign bit is always `0b0`, so it can be disregarded. Since the 8-bit exponent is followed by the 23-bit mantissa, if given a binary exponent $$p$$ and a binary mantissa $$m$$, the IEEE 754 representation of a floating-point number can be calculated as $$2^{23}p+m$$, since multiplying $$p$$ by $$2^{23}$$ shifts it 23 binary digits to the left. Reverse-engineering this formula yields the following equation for the values of the IEEE 754 representations of the mantissa $$m$$ and the exponent $$p$$ for a given number $$n$$:
...
For small values of $$x$$ (those in the range $$[0,1]$$), $$\log_2(1+x)\approx{x}$$. A corrective term, $$\mu$$, is added to this, yielding $$\log_2(1+x)\approx{x}+\mu$$. Since $$\frac{m}{2^{23}}$$ is in the range $$[0,1]$$, the equation can be simplified further as follows:
...
The optimal value of $$\mu$$ is the value that gives the smallest average error for numbers in the range $$[0,1]$$. To determine this value, first take the function $$f$$ of the difference between $$\log_2(1+x)$$ and $$x$$.
...
$$
f'(x)=\frac{\mathrm{d}}{\mathrm{d}x}\log_2(1+x)-x=\frac{1}{(1+x)\ln(2)}-1
$$

$$
\frac{1}{(1+x)\ln(2)}-1=0\implies{x}=\frac{1}{\ln(2)}-1\approx0.4427
$$

$$
f(\frac{1}{\ln(2)}-1)\approx0.0861
$$

This value represents the maximum error. Based on the shape of the graph, the optimal value of $$\mu$$ can be determined by taking half of this value.

$$
\mu=\frac{f(\frac{1}{\ln(2)}-1)}{2}\approx0.043
$$
...
$$-\frac{2^{23}p_y+m_y}{2}$$ is equivalent to `- (i >> 1)` in the code, which leaves $$\frac{3(2^{23})(127-\mu)}{2}\approx1597488310$$, which is approximately equal to the constant `0x5f3759DF` (the difference appears to come from rounding in the value of $$\mu$$).

### Newton's Method

[Newton's method](https://en.wikipedia.org/wiki/Newton%27s_method) is an iterative algorithm which produces successively better approximations to the roots of a real-valued function. In other words, every time we run our approximation through Newton's method with a function that has a root at the real value, it will become closer to the real value. The function $$f$$ that the code uses is $$f(y)=\frac{1}{y^2}-x$$, since if $$y$$ is a root of $$f$$, then $$y$$ is the inverse square root of $$x$$. This means that the value stored in `y` is our initial approximation $$x_0$$.

$$
0=\frac{1}{y^2}-x\implies{y}=\frac{1}{\sqrt{x}}
$$
...
$$
x_{n+1}=x_n-\frac{f(x_n)}{f'(x_n)}=x_n-\frac{\frac{1}{x_n^2}-x}{-\frac{2}{x_n^3}}=\frac{x_n(3-xx_n^2)}{2}
$$

Because of the first line of code, `x2` stores half of $$x$$ (the value that we are trying to calculate the multiplicative inverse square root of). Since `y` contains an approximation $$x_0$$, a better approximation $$x_1$$ can be calculated and stored in `y` with the fifth line of code.
...
Carmack, J., Duffy, R. A., & Dosé, J. (1999, December 2). _Quake III Arena_ [Source code]. [https://github.com/id-Software/Quake-III-Arena](https://github.com/id-Software/Quake-III-Arena).

Nemean. (2020, November 28). _Fast Inverse Square Root - A Quake III Algorithm_ [Video]. YouTube. [https://youtu.be/p8u_k2LIZyo](https://youtu.be/p8u_k2LIZyo)

Pompili, L. (2022, November 17). _Why does this approximation constant work?_ [Online forum post]. Mathematics Stack Exchange. [https://math.stackexchange.com/a/4579143](https://math.stackexchange.com/a/4579143).

Theys, M. (2202). _Floating Point_ [PowerPoint slides].
