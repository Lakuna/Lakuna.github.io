...
In μGL, shaders are represented by [`Shader`](https://ugl.lakuna.pw/classes/Shader.html) objects.
...
In μGL, shader programs are represented by [`Program`](https://ugl.lakuna.pw/classes/Program.html) objects. A `Program` can be created by passing in `Shader`s.
...
If you don't plan on reusing the shaders for anything else, it can be more convenient to instead use the `Program.fromSource` static method, which automatically handles creating and deleting the shaders for you.
