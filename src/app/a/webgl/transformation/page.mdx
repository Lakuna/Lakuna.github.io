...
Although translation is not a linear transformation, it is an affine transformation, so it can still be represented with matrices through the use of homogeneous coordinates. This is why it's necessary to use $$4\times4$$ matrices and 4D vectors to represent 3D transformations.
