...
2D textures in μGL are represented by [`Texture2d`](https://ugl.lakuna.pw/classes/Texture2d.html)s. To load an image into a texture, you can use [`fromImageUrl`](https://ugl.lakuna.pw/classes/Texture2d.html#fromimageurl) (which mirrors the behavior above) or [`fromImageUrlPromise`](https://ugl.lakuna.pw/classes/Texture2d.html#fromimageurlpromise), which returns a promise that resolves when the image is loaded.

```js
const texture = Texture2d.fromImageUrl(
	gl,
	"https://www.lakuna.pw/images/webgl-example-texture.png"
);
```
...
μGL abstracts away both binding points and texture units, so texture values can be passed directly to uniform names in the `draw` call.

```js
gl.fbo.draw(vao, { u_texture: texture });
```
...
μGL [`Texture`](https://ugl.lakuna.pw/classes/Texture.html)s have accessors that correspond to each parameter.
...
If you attempt to sample a μGL texture that isn't texture complete, a mipmap will automatically be generated for that texture first. You can also manually generate a mipmap with the [`generateMipmap`](https://ugl.lakuna.pw/classes/Texture.html#generatemipmap) method.
...
gl.texImage2D(gl.TEXTURE_2D, 0, gl.R8, 3, 2, 0, gl.RED, gl.UNSIGNED_BYTE, data);
```

μGL textures can have their format set via the [`format`](https://ugl.lakuna.pw/classes/Texture.html#format) accessor and the internal format and data type will be automatically determined. Then, data can be passed to the texture with the [`setMip`](https://ugl.lakuna.pw/classes/Texture.html#setmip) method.

```js
texture.format = TextureFormat.R8;
texture.setMip(data, 0, void 0, [0, 0, 3, 2]);
```
...
μGL automatically adjusts the unpack alignment as needed, but it can be overridden via a parameter on `setMip`.
