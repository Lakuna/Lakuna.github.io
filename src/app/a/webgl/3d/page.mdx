...
μGL has equivalent accessors [`doCullFace`](https://ugl.lakuna.pw/classes/Context.html#docullface) and [`cullFace`](https://ugl.lakuna.pw/classes/Context.html#cullface), respectively.
...
In μGL, depth testing can be enabled with the [`doDepthTest`](https://ugl.lakuna.pw/classes/Context.html#dodepthtest) accessor.
...
In μGL, the depth buffer is typically cleared by passing an extra value to [`clear`](https://ugl.lakuna.pw/classes/Framebuffer.html#clear) representing the value to clear to. It can also be set via the [`clearDepth`](https://ugl.lakuna.pw/classes/Context.html#cleardepth) accessor.

```js
gl.clear(true, 1); // `true` means "use previous value."
```
