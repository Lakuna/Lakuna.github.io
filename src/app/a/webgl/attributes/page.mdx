...
If you need to set an attribute's location explicitly in μGL, you can pass a map of attribute names to locations to the [`Program`](https://ugl.lakuna.pw/classes/Program.html) constructor or you can use a layout qualifier.
...
One of the primary features of μGL is that it completely abstracts away binding points from the developer. Binding points are managed so as to minimize the number of calls to the WebGL API, and wrapper objects can be used in any place where a binding point would be used in vanilla WebGL. The object that represents vertex buffers is [`VertexBuffer`](https://ugl.lakuna.pw/classes/VertexBuffer.html), and the constructor can be passed the data that should go in the buffer directly. The data can be modified at a later point via the [`data`](https://ugl.lakuna.pw/classes/VertexBuffer.html#data) accessor.
...
μGL represents VAOs with [`VertexArray`](https://ugl.lakuna.pw/classes/VertexArray.html)s. μGL abstracts away variable locations, so attribute names can be used to refer to attributes in the constructor or at a later time by using [`setAttribute`](https://ugl.lakuna.pw/classes/VertexArray.html#setattribute).

```js
const vao = new VertexArray(program, { a_position: { size: 2, vbo: buffer } });
```

μGL assumes that data is non-normalized, tightly-packed, and 3-dimensional by default. If all of these things are true for your use case, you can pass the buffer to the attribute directly.

```js
const vao = new VertexArray(program, { a_position: buffer });
```
...
Like when clearing the screen, μGL is explicit about which framebuffer is being modified by draw calls, so drawing is done with the framebuffer's [`draw`](https://ugl.lakuna.pw/classes/Framebuffer.html#draw) method. μGL automatically handles using the correct shader program and binding the correct VAO, and it draws all of the data in the VAO as triangles by default (although this can be overridden with arguments).
...
In μGL, element buffers are represented by [`ElementBuffer`](https://ugl.lakuna.pw/classes/ElementBuffer.html)s. A VAO's EBO can be set by passing it to the VAO's constructor or by using the [`ebo`](https://ugl.lakuna.pw/classes/VertexArray.html#ebo) accessor. When a `VertexArray` has an EBO, μGL automatically utilizes indexed drawing without requiring the user to modify their `draw` call.

```js
vao.ebo = new ElementBuffer(gl, indices);
```
